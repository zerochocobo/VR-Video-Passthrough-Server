...
# quality label -> --max-side (0 = original resolution)
_QUALITY_TO_MAX_SIDE = {"fast": 1280, "balanced": 1920, "hq": 0}
...
class TwoDvrPage(QWidget):
    ...
    def _quality_combo(self) -> QComboBox:
        combo = _fit_combo(QComboBox())
        combo.addItem("", "fast")
    ...
    def _single_tab(self) -> None:
        ...
        self.single_quality = self._quality_combo()
        ...
        self.single_labels = {
            key: _label() for key in ("video", "output", "model", "projection", "strength", "quality")
        }
        ...
        grid.addWidget(self.single_labels["quality"], 5, 0)
        grid.addWidget(self.single_quality, 5, 1, alignment=Qt.AlignLeft)
        grid.addWidget(self.single_time_mode, 6, 0, alignment=Qt.AlignRight)
    ...
    def _batch_tab(self) -> None:
        ...
        self.batch_quality = self._quality_combo()
        ...
        grid.setColumnStretch(1, 1)
        self.batch_labels = {
            key: _label() for key in ("directory", "model", "projection", "strength", "quality")
        }
        ...
        grid.addWidget(self.batch_labels["quality"], 4, 0)
        grid.addWidget(self.batch_quality, 4, 1, alignment=Qt.AlignLeft)
    ...
    def _render_args(self, model: QComboBox, projection: QComboBox, strength: QComboBox, quality: QComboBox) -> list[str]:
        max_side = _QUALITY_TO_MAX_SIDE.get(str(quality.currentData()), 1920)
        return [
    ...
    def run_single(self) -> None:
        ...
        args.extend(self._render_args(
            self.single_model, self.single_projection, self.single_strength, self.single_quality,
        ))
        if str(self.single_time_mode.currentData()) == "segments":
    ...
    def run_batch(self) -> None:
        ...
        args.extend(self._render_args(
            self.batch_model, self.batch_projection, self.batch_strength, self.batch_quality,
        ))
    ...
    def retranslate(self) -> None:
        ...
        self.single_labels["quality"].setText(self.i18n.t("twodvr.quality"))
        ...
        self.batch_labels["quality"].setText(self.i18n.t("twodvr.quality"))
        for combo in (self.single_model, self.batch_model):
            combo.setItemText(0, self.i18n.t("twodvr.model_small"))
            combo.setItemText(1, self.i18n.t("twodvr.model_base"))
            combo.setItemText(2, self.i18n.t("twodvr.model_small_hd"))
            combo.setItemText(3, self.i18n.t("twodvr.model_base_hd"))
        ...
        for combo in (self.single_quality, self.batch_quality):
            combo.setItemText(0, self.i18n.t("twodvr.quality_fast"))
            combo.setItemText(1, self.i18n.t("twodvr.quality_balanced"))
            combo.setItemText(2, self.i18n.t("twodvr.quality_hq"))
