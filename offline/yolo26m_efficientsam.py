"""YOLO26m + EfficientSAM prepass helpers for MatAnyone2 offline mode."""
...
class Yolo26mEfficientSamMasker:
    def __init__(
        self,
        model_dir: Path,
        sam_model_dir: Path,
        ...
        self.sam_model_dir = sam_model_dir
        ...
        self.sam = ort.InferenceSession(str(sam_model_dir / sam_model), sess_options=opts, providers=providers)
        print(
            f"[offline] YOLO26m+EfficientSAM loaded dir={model_dir} sam_dir={sam_model_dir} yolo={yolo_model} sam={sam_model} "
    ...
    def _sanity_check_yolo_outputs(self, yolo_model: str) -> None:
        ...
        if max_sig < 0.05:
            print(
                ...
                f"Switch --y26es-yolo-model to yolo26m_model.onnx (fp32) or run on CPU.",
                flush=True,
            )
...
def sample_points(args, src: Path, source_fps: float, fps: float, target: int) -> list[int]:
    scan = str(getattr(args, "y26es_scan", "hybrid"))
    interval = float(getattr(args, "y26es_scan_interval_sec", 1.0))
...
def precompute_segment_masks(args, src: Path, dec, source_fps: float, fps: float, target: int, cfr_source_index: Callable[[int, float, float], int]):
    if getattr(args, "engine", "") != "matanyone2_onnx" or getattr(args, "mask", ""):
        return {}, [0]
    if bool(getattr(args, "y26es_subprocess", True)) and not bool(getattr(args, "_y26es_child", False)):
    ...
    masker = Yolo26mEfficientSamMasker(
        Path(getattr(args, "y26es_model_dir", config.ROOT / "models" / "yolo26m")).resolve(),
        Path(getattr(args, "y26es_sam_model_dir", config.ROOT / "models" / "efficientsam")).resolve(),
        provider=str(getattr(args, "y26es_provider", "cuda")),
        yolo_model=str(getattr(args, "y26es_yolo_model", "yolo26m_model.onnx")),
        sam_model=str(getattr(args, "y26es_sam_model", "efficientsam_s.onnx")),
        yolo_size=int(getattr(args, "y26es_yolo_size", 640)),
        score_threshold=float(getattr(args, "y26es_score_threshold", 0.35)),
        nms_threshold=float(getattr(args, "y26es_nms_threshold", 0.6)),
        box_expand=float(getattr(args, "y26es_box_expand", 0.08)),
        top_k=int(getattr(args, "y26es_top_k", 0)),
        binarize_mask=bool(getattr(args, "y26es_binarize_mask", True)),
        mask_erode_px=int(getattr(args, "y26es_mask_erode_px", 1)),
        max_box_area=float(getattr(args, "y26es_max_box_area", 0.50)),
        cross_eye_area_ratio=float(getattr(args, "y26es_cross_eye_area_ratio", 1.5)),
    )
    debug_dir = Path(args.y26es_debug_dir).resolve() if getattr(args, "y26es_debug_dir", "") else None
    if debug_dir:
        debug_dir.mkdir(parents=True, exist_ok=True)
    print(
        f"[offline] YOLO26m+EfficientSAM prepass samples={len(scan_points)} "
        f"scan={getattr(args, 'y26es_scan', 'hybrid')} max_segment_frames={max_segment_frames} "
    ...
    for n, start in enumerate(scan_points, 1):
        ...
        active = (
            infos[0]["union_area_ratio"] >= float(getattr(args, "y26es_active_min_area_ratio", 0.001))
            or infos[1]["union_area_ratio"] >= float(getattr(args, "y26es_active_min_area_ratio", 0.001))
        )
        ...
        print(
            f"[offline] Y26+ES prepass {n}/{len(scan_points)} frame={start} src_idx={src_idx} "
    ...
    if filled_gaps:
        print(
            f"[offline] YOLO26m+EfficientSAM filled short inactive gaps frames={filled_gaps} "
    ...
    starts = _planned_starts(
        ...
        max_segment_frames,
        min_segment_frames,
        bool(getattr(args, "y26es_cut_on_count_change", True)),
        bool(getattr(args, "y26es_cut_every_active_sample", False)),
        scene_min_frames,
    )
    ...
    if not masks_by_start and bool(getattr(args, "y26es_fail_on_empty", True)):
        raise RuntimeError(
            "YOLO26m + EfficientSAM prepass found no active person masks. "
            "Try lowering --y26es-score-threshold, using --y26es-debug-dir, or falling back to MatAnyone2 slow/SAM3."
        )
...
def precompute_segment_masks_subprocess(args, src: Path, source_fps: float, fps: float, target: int):
    tmp_dir = config.ROOT / "debug_output" / "_y26es_prepass"
    tmp_dir.mkdir(parents=True, exist_ok=True)
    result_path = tmp_dir / f"y26es_prepass_{int(time.time() * 1000)}_{id(args)}.npz"
    ...
    cmd += [
        ...
        "--y26es-model-dir", str(Path(args.y26es_model_dir).resolve()),
        "--y26es-sam-model-dir", str(Path(args.y26es_sam_model_dir).resolve()),
        "--y26es-provider", str(args.y26es_provider),
        "--y26es-yolo-model", str(args.y26es_yolo_model),
        "--y26es-sam-model", str(args.y26es_sam_model),
        "--y26es-yolo-size", str(args.y26es_yolo_size),
        "--y26es-score-threshold", str(args.y26es_score_threshold),
        "--y26es-nms-threshold", str(args.y26es_nms_threshold),
        "--y26es-box-expand", str(args.y26es_box_expand),
        "--y26es-top-k", str(args.y26es_top_k),
        "--y26es-mask-erode-px", str(args.y26es_mask_erode_px),
        "--y26es-max-box-area", str(args.y26es_max_box_area),
        "--y26es-cross-eye-area-ratio", str(args.y26es_cross_eye_area_ratio),
        "--y26es-scan", str(args.y26es_scan),
        "--y26es-scan-interval-sec", str(args.y26es_scan_interval_sec),
        "--y26es-active-min-area-ratio", str(args.y26es_active_min_area_ratio),
        "--y26es-gap-fill-frames", str(args.y26es_gap_fill_frames),
        "--y26es-fill-boundaries" if bool(getattr(args, "y26es_fill_boundaries", True)) else "--no-y26es-fill-boundaries",
        "--y26es-scene-aware-fill" if bool(getattr(args, "y26es_scene_aware_fill", True)) else "--no-y26es-scene-aware-fill",
        ...
        "--frames", str(target),
        "--fps", str(fps),
        "--y26es-prepass-out", str(result_path),
    ]
    if not bool(getattr(args, "y26es_binarize_mask", True)):
        cmd += ["--no-y26es-binarize-mask"]
    if getattr(args, "y26es_debug_dir", ""):
        cmd += ["--y26es-debug-dir", str(Path(args.y26es_debug_dir).resolve())]
    if not bool(getattr(args, "y26es_cut_on_count_change", True)):
        cmd += ["--no-y26es-cut-on-count-change"]
    if bool(getattr(args, "y26es_cut_every_active_sample", False)):
        cmd += ["--y26es-cut-every-active-sample"]
    if not bool(getattr(args, "y26es_fail_on_empty", True)):
        cmd += ["--no-y26es-fail-on-empty"]
    print("[offline] YOLO26m+EfficientSAM prepass subprocess=" + subprocess.list2cmdline(cmd))
    run_hidden_streaming(cmd, check=True, exit_label="offline-y26es")
